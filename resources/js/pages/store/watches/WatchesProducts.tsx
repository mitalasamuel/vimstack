...
export default function WatchesProducts({
  ...
  const applyFilters = () => {
    ...
    router.visit(route('store.products', storeSlug), {
      data: params,
      preserveState: true,
      preserveScroll: true
    });
        ...
        <div className="bg-slate-50 py-4 border-b border-slate-200">
          <div className="container mx-auto px-4">
            <div className="flex items-center text-sm">
              <Link href={route('store.home', storeSlug)} className="text-slate-500 hover:text-amber-600">Home</Link>
                  ...
                  {products.map((product) => (
                    viewMode === 'list' ? (
                      <div key={product.id} className="bg-white border border-slate-200 hover:border-slate-300 transition-colors duration-300">
                        <div className="flex">
                          <Link href={route('store.product', { storeSlug, id: product.id })} className="w-48 aspect-square flex-shrink-0 relative overflow-hidden bg-slate-50">
                            <img 
                          ...
                          <div className="flex-1 p-6">
                            <div className="flex justify-between items-start h-full">
                              <div className="flex-1">
                                ...
                                <Link href={route('store.product', { storeSlug, id: product.id })}>
                                ...
                                <div className="flex items-center mb-4">
                                  {product.sale_price ? (
                                    <>
                                      <span className="text-2xl font-light text-slate-900">{formatCurrency(product.sale_price, storeSettings, currencies)}</span>
                                      <span className="text-lg text-slate-500 line-through ml-3 font-light">{formatCurrency(product.price, storeSettings, currencies)}</span>
                                      <span className="ml-3 bg-amber-100 text-amber-800 text-xs font-medium px-2 py-1 uppercase tracking-wider">
                                        {Math.round(((product.price - product.sale_price) / product.price) * 100)}% OFF
                                      </span>
                                    </>
                                  ) : (
                                    <span className="text-2xl font-light text-slate-900">{formatCurrency(product.price, storeSettings, currencies)}</span>
                                  )}
                                </div>
                                <div className="flex items-center mb-4">
                                  {product.stock > 0 ? (
