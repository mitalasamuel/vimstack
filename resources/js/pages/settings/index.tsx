...
export default function Settings() {
  ...
  const { systemSettings = {}, cacheSize = '0.00', timezones = {}, dateFormats = {}, timeFormats = {}, paymentSettings = {}, webhooks = [], auth = {}, flash } = usePage().props as any;
  ...
  const sidebarNavItems = allSidebarNavItems.filter(item => {
    ...
    if (auth.user && auth.user.type === 'company') {
      // Only allow system settings, email settings, brand settings, currency settings, payment settings, webhook settings, and settings
      return ['manage-system-settings', 'manage-email-settings', 'manage-brand-settings', 'manage-currency-settings', 'manage-payment-settings', 'manage-webhook-settings', 'settings'].includes(item.permission);
    }
    return false;
  });
    ...
    >
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          {/* {auth.permissions?.includes('manage-webhook-settings') && (
